// Adapted from CardputerLLM/src/setup/key_setup.cpp.
// Form HTML retitled "Verbatim"; done-page mentions notes list instead of
// chat; save-path string updated. Behaviour and trust model identical.
#include "key_setup.h"
...
<title>Cardputer &middot; set api key</title>
<style>
...
<div class="frame">
  ...
  <h1>set api key</h1>
  <div class="rule"></div>
  <p>Paste your OpenRouter API key. The device saves it to the SD card and continues to the notes list.</p>
...
<div class="frame">
  ...
  <h1>saved.</h1>
  <div class="rule"></div>
  <p>Key written to <code>/Cardputer/openrouter.txt</code>.</p>
  <p>Cardputer is continuing to the notes list.</p>
...
void drawWaitingScreen() {
    ...
    // "http://" small, above the IP.
    M5Cardputer.Display.setFont(&fonts::Font0);
    M5Cardputer.Display.setTextColor(kDim, TFT_BLACK);
    const char* scheme = "http://";
    int schemeW = M5Cardputer.Display.textWidth(scheme);
    M5Cardputer.Display.setCursor((240 - schemeW) / 2, 40);
    M5Cardputer.Display.print(scheme);

    // IP itself, big and centered. The previous layout used Font2 textSize 2
    // for "http://<ip>" which on a 240px screen silently clips IPs whose
    // last octet is three digits. Render just the IP at size 2 (or 1 if it
    // would still overflow at size 2, which only happens for the rare
    // 255.255.255.255-style worst case) and put the http:// label above.
    M5Cardputer.Display.setFont(&fonts::Font2);
    int sz = 2;
    M5Cardputer.Display.setTextSize(sz);
    int w = M5Cardputer.Display.textWidth(ip.c_str());
    const int sideMargin = 10;
    if (w > 240 - sideMargin * 2) {
        sz = 1;
        M5Cardputer.Display.setTextSize(sz);
        w = M5Cardputer.Display.textWidth(ip.c_str());
    }
    int ipY    = (sz == 2) ? 52 : 56;
    int ipH    = (sz == 2) ? 22 : 14;
    int boxPad = 6;
    int boxX   = (240 - w) / 2 - boxPad;
    int boxY   = ipY - 3;
    int boxW   = w + boxPad * 2;
    int boxH   = ipH;
    if (boxX < 2) { boxX = 2; }
    if (boxX + boxW > 238) { boxW = 238 - boxX; }
    ...
    M5Cardputer.Display.setCursor((240 - w) / 2, ipY);
    ...
    M5Cardputer.Display.setCursor(8, 98);
