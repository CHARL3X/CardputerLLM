// Adapted from CardputerLLM/src/setup/wifi_setup.cpp.
// Only diff vs. source: the "saved to ..." hint string references the
// Verbatim path, and sdcfg::appendWiFiCred now writes to /Cardputer/wifi.txt
// because the sd_config implementation is namespaced. Behaviour identical.
...
void drawConnected(const String& ssid, const String& ip) {
    ...
    boot_ui::hintBar("saved to /Cardputer/wifi.txt");
}
